...
LLM 서비스 기획자와 개발자를 위한 인터랙티브 성능 산출 도구입니다. 모델 파라미터, 양자화 비트, 하드웨어 사양을 기반으로 필요한 VRAM과 처리량(TPS), 동시 접속자 수를 예측합니다. 단일 장비를 넘어 **멀티 GPU 클러스터**환경의 자원 시뮬레이션을 지원합니다.
...
- **하드웨어 사양 업데이트**: 신규 출시되는 NVIDIA Blackwell(B100/B200), AMD MI 시리즈, **HP AI 워크스테이션**의 세부 사양을 데이터베이스에 정기 반영합니다.
- **멀티 노드 연산 고도화**: 클러스터 구성 시 발생하는 네트워크 오버헤드를 반영한 `Interconnect Efficiency` 인자 추가를 검토 중입니다.
...
본 프로젝트의 모든 결과물(소스 코드, 디자인, 기술 문서)에 대한 저작권은 **Selim**에게 있습니다. **무단 전재, 배포, 상업적 이용 및 무단 수정 후 배포를 엄격히 금지합니다.** 오픈 소스 형태로 공개되어 있더라도 모든 권리는 저작권자에게 귀속됨을 명시합니다.
